#=
================================================================================
 functions_2_build_TS_model_w_Kron_Linear.jl  —  TSC-DCOPF (Taylor Pe)
================================================================================
 Phase 2: shared builders are in `functions_2_build_TS_model_w_Kron_shared.jl`.
 This file keeps TSC-DCOPF-only physics:
   • P_init linearised around θ
   • Pe → eq_const_tsredlinear_Pe_taylor! (Taylor around δ_ref from pre-solve)
   • Δω → eq_const_tsredlinear_Δω_swingeq_generic_modified!

 Alternative Pe / Δω variants: experimental_tsredlinear_pe_variants.jl (not loaded).
================================================================================
=#
...
# ==================================================================================
function Make_Dynamic_Model_tsredlinear!(
    model::JuMP.Model, 
    ...
    if ts_fault_details[:fault_type] == "SC"
        ...
        if ts_fault_details[:sc][:fault_location] == "bus"
            ...
            begin
                ...
                # =================================================================================
                model, dyn_model_dict = Define_Initial_Condition_4_tsredlinear!(model, ts_input_param, dyn_model_dict, DGEN_mod, DGEN_DYN_mod, active_gen, base_MVA, opf_dict[:vars][:θ], opf_dict[:vars][:P_g]; mech_power_mode=mech_power_mode)

                # =================================================================================
                # Model the Fault
                # =================================================================================
                model, dyn_model_dict = Define_Fault_Dynamic_Model_tsredlinear!(model, dyn_model_dict, active_gen, DGEN_DYN_mod, t_step, t_window_fault, Yred_fault, dyn_model_dict[:vars][:δ], Δω_0, ω_syn, δ_tol, δ_ref)
            end
            ...
            if ts_fault_details[:sc][:bus][:disconnect_branch] == true # A branch will be disconnected
                ...
                begin
                    ...
                    model, dyn_model_dict = Define_PostFault_Dynamic_Model_tsredlinear!(model, dyn_model_dict, active_gen, DGEN_DYN_mod, t_step, t_window_postf, Yred_postf, last_var_Pe_tf, last_var_δ_tf, last_var_Δω_tf, ω_syn, δ_tol, δ_ref)
                end
    ...
    elseif ts_fault_details[:fault_type] == "OB"
        ...
        begin
            ...
            # Same outage-style timing as GL, but with a pure topology change on the
            # Kron network copy before rebuilding Ybus/Yred for the linearised path.
            ...
            model, dyn_model_dict = Define_Initial_Condition_4_tsredlinear!(
                model, ts_input_param, dyn_model_dict, DGEN_mod, DGEN_DYN_mod,
                active_gen, base_MVA, opf_dict[:vars][:θ], opf_dict[:vars][:P_g];
                mech_power_mode=mech_power_mode)

            model, dyn_model_dict = Define_Fault_Dynamic_Model_tsredlinear!(
...
function Define_Initial_Condition_4_tsredlinear!(model::JuMP.Model,
    ...
    if mech_power_mode == USE_PM
        ...
        store_ts_scalar_var_bounds!(dyn_model_dict, :P_m, result, :ineq_const_P_m_lower, :ineq_const_P_m_upper)
    end

    #-------------------------------------------------------------------------
    #                          Equality Constraints (always-on physics)
    #-------------------------------------------------------------------------
    # Linearized Pe–δ–θ link always; no classical Q_g init on this path.
    # Pin P_m = P_g only when mechanical power is an independent variable.
    dyn_model_dict[:eq_const][:eq_const_P_init] = eq_const_tsredlinear_initial_active_power!(
        model, θ, P_g, δ, DGEN, DGEN_DYN, active_gen)
    if mech_power_mode == USE_PM
...
function Define_Fault_Dynamic_Model_tsredlinear!(model::JuMP.Model,
    dyn_model_dict::OrderedDict{Symbol, Any},
    active_gen::Vector{Int64},
    DGEN_DYN::DataFrame,
    Δt::Float64,
...
# Function used to define the variables that vary in time in the fault period
function Define_PostFault_Dynamic_Model_tsredlinear!(model::JuMP.Model,
    dyn_model_dict::OrderedDict{Symbol, Any},
    active_gen::Vector{Int64},
    DGEN_DYN::DataFrame,
    Δt::Float64,
