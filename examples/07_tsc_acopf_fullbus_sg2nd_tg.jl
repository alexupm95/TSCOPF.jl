#=
================================================================================
 07 — TSC-ACOPF — FULL_BUS + CLASSICAL_2ND + turbine governor (TGOV1)
================================================================================
  ...
  Example 06 plus primary frequency control. Mechanical power stops being a
  constant through the swing: each unit gets a TGOV1 lead-lag governor driven by
  its own speed deviation, so P_m follows the frequency excursion back toward the
  set-point instead of sitting at its pre-fault value.

  Note the input file changes to `gen_dynamic_data_full.csv`. The minimal
  five-column `gen_dynamic_data.csv` used in example 06 has no droop or time
  constants; the governor needs R, T1, T2, T3 (parser at
  _manage_inputs/functions_2_read_input_files.jl:22-106).

  The governor is legal on a classical machine — unlike the AVR, which needs a
  field winding and therefore DQ_4TH (engine.jl:214-216). It does require
  `mech_power_mode = USE_PM` (engine.jl:226-229) and `network_form = FULL_BUS`
  (engine.jl:231-235), both already true on this path.

  `governor_limiter` picks how valve saturation is modelled:
      GOV_NO_LIMIT     no saturation — the valve can exceed its physical travel
      GOV_SMOOTH       smooth-max/min composition; differentiable, NLP-friendly
      GOV_HARD_BOUND   hard bounds on the valve variable
  GOV_SMOOTH is used here: it keeps the saturation physics without handing Ipopt
  a nondifferentiable kink.
...
const ts_limits = TsBoundLimitsConfig(
    ...
    # ── Governor pre-fault set-point ─────────────────────────────────────────
    P_ref_source = :dgen_pg_limits,   # :dgen_pg_limits = take the P_ref box from DGEN Pmin/Pmax ÷ base_MVA
    # ── Governor research boxes on the valve / mechanical trajectories ───────
    # Inactive here. Saturation physics is unchanged by these — they only add
    # ≤-rows (and therefore duals) on Pv and Pm through the two windows.
...
const ts_builder = TsBuilderConfig(
    ...
    # Governor pre-fault set-point box (values from P_ref_source above). Off by
    # default like every optional box: switching it on adds a ≤-row per unit and
    # can make the problem infeasible if the optimal P_ref sits outside DGEN's
    # Pmin/Pmax. Turn it on deliberately, and watch the objective.
    bound_P_ref = false,      # false = P_ref is a free variable, unconstrained by DGEN limits
    # Governor research boxes (values from Pv_*/Pm_* above)
...
const dyn_model_cfg = DynModelConfig(
    ...
    network_form            = FULL_BUS,         # required by include_governor; also keeps every bus in the model
    mech_power_mode         = USE_PM,           # required by include_governor: P_m must be its own variable to be governed
    ...
    constrain_Δω        = false,            # false = no corridor on Δω_i − Δω_COI; the governor is what makes it meaningful
...
const transient_cfg = TransientConfig(
    ...
    # Full header needed: the governor reads R, T1, T2, T3 per unit.
    gen_dynamic_filename = "gen_dynamic_data_full.csv",   # 17-column header; the minimal file has no droop constants
...
println("Solving ACOPF warm start + TSC-ACOPF (FULL_BUS, classical 2nd order + TGOV1)…")
...
println("Governor    : mechanical_power.csv + valve trajectories under Transient_Stability/CSV")
