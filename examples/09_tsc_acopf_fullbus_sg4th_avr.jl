#=
================================================================================
 09 — TSC-ACOPF — FULL_BUS + DQ_4TH + AVR (exciter)
================================================================================
  ...
  Example 08 plus automatic voltage regulation. A first-order exciter (T_exc,
  K_exc from the dynamic CSV) drives the field voltage E_fd from the error
  between the terminal voltage and the set-point V_ref, so E'q is no longer left
  to decay freely — the machine actively defends its terminal voltage through the
  fault and the recovery.

  AVR is DQ-only, and validation says so directly: `include_avr = true` requires
  `gen_order = DQ_4TH` (engine.jl:214-216). A classical machine holds a constant
  EMF and has no field winding for the exciter to act on, which is why there is
  no "2nd order + AVR" example in this folder.

  Two new knob families appear here:
    • `V_ref_min_pu` / `V_ref_max_pu` with `bound_V_ref` — a box on the
      pre-fault voltage set-point, which is a decision variable.
    • `E_fd_unlim_tf` / `E_fd_unlim_tpf` with their toggles — research boxes on
      the *unlimited* field voltage. The exciter's own saturation physics is
      built regardless; these only add ≤-rows (and duals) on the pre-saturation
      signal.

  Knob families omitted here because this example runs without them, each with
  the example that does show them: governor valve limits and boxes (07, 10),
  GFM boxes and limits (12).
================================================================================
=#
...
const ts_limits = TsBoundLimitsConfig(
    ...
    # ── AVR pre-fault set-point ──────────────────────────────────────────────
    # Read only when bound_V_ref is on. Setting max = Inf builds the lower row
    # only, which is the usual way to keep V_ref off zero without capping it.
    V_ref_min_pu = 0.8,   # lower bound on the AVR voltage set-point [pu]
    ...
    # ── AVR research boxes on the unlimited field voltage (inactive) ─────────
    # The exciter's saturation physics is built either way; these add ≤-rows on
    # the pre-saturation signal so its shadow price becomes observable.
...
const ts_builder = TsBuilderConfig(
    ...
    # AVR pre-fault set-point box (values from V_ref_min_pu / V_ref_max_pu).
    # Off by default like every optional box: switching it on adds a ≤-row per
    # unit and can make the problem infeasible if the optimal V_ref sits outside
    # [0.8, 1.2]. Turn it on deliberately, and watch the objective.
    bound_V_ref = false,      # false = V_ref is a free variable, unconstrained by the limits above
    ...
    # AVR research boxes (values from E_fd_unlim_* above)
...
const dyn_model_cfg = DynModelConfig(
    ...
    include_governor        = false,            # legal to set true here; see example 11
...
const transient_cfg = TransientConfig(
    ...
    # DQ_4TH needs the full machine header; the AVR additionally reads T_exc, K_exc.
...
println("Solving ACOPF warm start + TSC-ACOPF (FULL_BUS, dq 4th order + AVR)…")
...
println("AVR         : field-voltage trajectories under Transient_Stability/CSV")
