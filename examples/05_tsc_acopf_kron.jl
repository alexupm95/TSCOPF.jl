#=
================================================================================
 05 — TSC-ACOPF — KRON_REDUCED + CLASSICAL_2ND
================================================================================
  ...
  Solver:   Ipopt (NLP). TSC-ACOPF forbids HiGHS and Gurobi.
  Results:  RESULTS/Results - <timestamp>/{Dispatch,Transient_Stability}/

  The classical formulation: constant-voltage-behind-transient-reactance machines
  on a Kron-reduced network. Loads are folded into Y_red as constant admittance
  and every non-generator bus is eliminated, so the dynamic model sees only the
  internal machine nodes. Cheap, and the historical baseline the FULL_BUS models
  ...
  instead of pinning it to the dispatch P_g, and `:coi_box` states the stability
  constraint directly as a corridor around the centre of inertia.

  Not available on this avenue: AVR (needs DQ_4TH, engine.jl:214-216), governor
  (needs FULL_BUS, engine.jl:231-235), GFM (needs FULL_BUS + DQ_4TH), ZIP loads
  (folded into Y_red; a non-default split only earns a warning).
...
println("Solving TSC-ACOPF (Kron-reduced, classical 2nd order)…")
