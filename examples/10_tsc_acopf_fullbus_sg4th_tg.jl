#=
================================================================================
 10 — TSC-ACOPF — FULL_BUS + DQ_4TH + turbine governor (TGOV1)
================================================================================
  ...
  Example 08 plus primary frequency control, and the counterpart to example 07:
  the same TGOV1 governor, now on a 4th-order machine. Comparing 07 against 10
  isolates what the flux-decay states change about the governor's job, since the
  disturbance and the control parameters are identical.

  The governor acts on the mechanical side and the AVR on the electrical side, so
  they are independent switches — example 11 turns both on. Requirements
  (engine.jl:226-235): `mech_power_mode = USE_PM` and `network_form = FULL_BUS`,
  both already forced by DQ_4TH.
  ...
  the example that does show them: AVR set-points and boxes (09, 11), GFM boxes
  and limits (12).
================================================================================
=#
...
const dyn_model_cfg = DynModelConfig(
    ...
    include_avr             = false,            # legal to set true here; see example 11
...
const transient_cfg = TransientConfig(
    ...
    # DQ_4TH needs the full machine header; the governor additionally reads R, T1, T2, T3.
...
println("Solving ACOPF warm start + TSC-ACOPF (FULL_BUS, dq 4th order + TGOV1)…")
